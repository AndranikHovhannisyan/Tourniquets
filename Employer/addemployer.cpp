...
void addEmployer::employerDepartmentPositionEmployerIdSave(int rowNumber)
{
    ...
    if (ui->department->currentIndex() >= 0 && ui->position->currentIndex() >= 0){
    ...
    if (ui->employerId->currentIndex() >= 0){
...
void addEmployer::findFreeEmployerIds(int employerId)
{
    ...
    employerIdModel->setQuery("SELECT DISTINCT ei.emp_number, ei.id_type " \
